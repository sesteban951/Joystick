...
{   
    // If Joystick is not connected or file descriptor is invalid, return early
    if (this->joy_connected == false || fd < 0) {
        std::cerr << "No joystick connected, cannot update." << std::endl;
        return;
    }

    // Joystick connected, update the axes and buttons
    else {
